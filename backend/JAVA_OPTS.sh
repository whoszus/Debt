...
MEM_OPTS="-Xms64m -Xmx512m -XX:NewRatio=1"

if [[ "$JAVA_VERSION" < "1.8" ]]; then
  MEM_OPTS="$MEM_OPTS -XX:PermSize=128m -XX:MaxPermSize=512m"
else         
  MEM_OPTS="$MEM_OPTS -XX:MetaspaceSize=64m -XX:MaxMetaspaceSize=256m"
fi
...
#GC_OPTS="$GC_OPTS -XX:ParallelGCThreads=6 -XX:ConcGCThreads=3"
